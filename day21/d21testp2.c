...
int main() {
	// ##################### FILENAME #################
	const char* file_name = "in_test.txt";
			...
			if(!monkeys[i].knowns_his_num) {
				...
				if(monkeys[ops[0]].knowns_his_num && monkeys[ops[1]].knowns_his_num) {
					...
					case '/':
						...
						divisibility = divisibility && (0 == monkeys[ops[0]].number % monkeys[ops[1]].number);
						monkeys[i].number = monkeys[ops[0]].number / monkeys[ops[1]].number;
						break;

					case '=':
						printf("%lld == \n%lld\n", monkeys[ops[0]].number, monkeys[ops[1]].number);

						monkeys[i].number = monkeys[ops[0]].number == monkeys[ops[1]].number;
						assert(monkeys[i].number);
						break;
	...
	printf("root's finds: %s\n", monkeys[root].number ? "equality" : "inequality");
	printf("divisibility %s respected\n", divisibility ? "IS" : "is NOT");
