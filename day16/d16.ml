...
let rec solve_d16 (time_left : int) (position :int) (open_vaves : Bitfield.t) (is_elephant :bool) =
  let current_valve = tunel_valve_graph.(position) in
  if time_left = 0 then
    if is_elephant 
      then 0
      else solve_d16 p2_total_time (code_to_int 'A' 'A') open_vaves true
  else 
    if Hashtbl.mem posible_states (time_left, position, open_vaves, is_elephant) then 
      Hashtbl.find posible_states (time_left, position, open_vaves, is_elephant)
    else
      let posible_reliefs = List.map (fun (vlv) -> solve_d16 (time_left - 1) vlv open_vaves is_elephant) current_valve.tunnels in
      let turn_on_valve_relief = if not (Bitfield.get open_vaves position) && current_valve.flow_rate > 0
        ...
        current_valve.flow_rate* (time_left - 1) + solve_d16 (time_left - 1) position new_open_valves is_elephant
        else 0 in
      let answer = list_max (turn_on_valve_relief :: posible_reliefs) in
      Hashtbl.add posible_states (time_left, position, open_vaves, is_elephant) answer;
      answer

let aa = code_to_int 'A' 'A' 
...
let () =
  ...
  (* part 1 
  let ans1 = solve_d16 30 aa Bitfield.empty true
  ...
  *)
  (* part 2 *)
let ans2 = solve_d16 p2_total_time aa Bitfield.empty false
